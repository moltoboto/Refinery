...
| # | Item | Est. | Notes |
|---|------|------|-------|
| P1 | **v2.61 — Artifact-header cleanup** (active next) | 1–2 hrs | Per Tom's screenshot: in `renderArtifactShell` remove the eyebrow (`SOURCE·DATE`) + meta (`type·size`); title = one line, serif (**keep Lora**); kill the blank space above the content; **all artifacts**. Font + scope already OK'd. (Numbering fix already shipped in v2.60.) (2026-07-01) |
| P2 | **Executive-summary format → reusable skill** (active next) | 1 session | Format proposed: MD, `Bottom Line` / `Core Message` / `Key Takeaways` (numbered, bold lead) / `Why It Matters`; example at `~/Downloads/2026_Agent_Memory_Systems_ExecSummary.md`. **AWAITING Tom's yes/tweaks**, then build a skill to generate this from any article. (2026-07-01) |
| H1 | **Accent highlight colors for important sections (HTML captures)** | 1 session | Tom wants important passages highlighted in an accent color (the Substack orange/"pink" look) in captured HTML artifacts. Content model: future captures = HTML (rich, highlights); exec summaries = MD (plain). Add a highlight style + a convention/marker. (2026-07-01) |
| T1 | **Apply cleaned feed list to TOR** | 30 min + decisions | `subscriptions-cleaned.opml` is a draft. TOR OPML import only ADDS — to apply the cuts, unsubscribe the removed feeds in TOR (Yahoo News, Fox News, Seeking Alpha, Motley Fool, Mad Money, Yahoo Finance, Fox Business, 22 YouTube) or delete-all + re-import. Also verify Reuters RSS still live (discontinued ~2020). Gated on F1 tickers / R1 Reddit / X1 `[XX]` numbers. (2026-07-01) |
| I1 | **Delete duplicate `[01] Inbox ` (trailing space) Drive folder** | 5 min | Two inbox folders exist in the Refinery Drive folder: `[01] Inbox` (where mail lands, v2.58) and a stray `[01] Inbox ` with a trailing space. Delete the stray to avoid split/confusion. (2026-07-01) |
| M1 | **Morning Brief — daily AM digest** | 1 session | Once-a-day morning digest clustering OVERNIGHT movement on Tom's top-10 holdings (per-ticker feeds, F1) into a tight per-name headline + one line — optionally + the day's top overnight stories. LLM-generated (extends 2b daily-brief) or feed-driven; viewable/delivered each morning. Ties F1 (tickers) + 2b (brief). **NEEDS: Tom's 10 tickers.** (2026-07-01) |
| IG1 | **Infographics cleanup + handling — DISCUSS** | discuss + ~1 session | `[05] Infographics` is messy: ~49 md + ~120 images (jpg/png/webp) + a `.JPEG_Archive`. Images aren't md/html so the sync excludes them → notes reference missing images (low value in Refinery). Decide: (a) VIEW infographics in Refinery (sync images, render as image artifacts) or (b) keep reference-only (Obsidian/Drive). Plus de-dupe/organize (wisdomware-filer / archive-pruner). (2026-07-01) |
| V1 | **Long-video intake workflow (NotebookLM?) — DISCUSS** | discuss + ~1 session | Hour-long session videos: too large for Refinery + not readable. Proposed: transcribe (captions/Whisper) → LLM summary → file as a Wisdomware `.md` (syncs to Refinery for reading); raw video stays link-only. NotebookLM fits the INTERACTIVE deep-dive per Tom's tool model (upload video/transcript → Q&A / briefing / audio). Likely both: NotebookLM for exploration + an auto-summary `.md` for the reading queue. (2026-07-01) |
...
| E1 | **Executive Summary layout — top ~3 lines render above the visible window** (#8) | 1–2 hrs | `summarizeVisible` already does `pane.scrollTo(0,0)`, so NOT a simple scroll reset. LEAD: **duplicate element IDs** — `readingPane`/`readingContent`/`readingEmpty` each appear TWICE (index.html ~1096 and ~1401; invalid HTML). `getElementById` returns the first, so scroll/injection can hit the wrong pane depending on which layout is visible. Fix: figure out why there are two reading-pane blocks (responsive layouts?), de-dup or scope the IDs, harden scroll-to-top. **Needs on-device verification (Tom).** Tom: "I have a format for the exec summary; clean up the layout." (2026-07-01) |
