...
|---|------|------|----------------|
| W1 | **Wisdomware reading: inline-render `.md`/`.html` + swipe in Artifacts** | 1–2 hrs | NEXT BUILD (design locked 2026-06-29). Fixes the iPad "trapped in Artifacts" problem at the root: artifacts render in an `<iframe>` so finger-swipe can't be captured (deliberate skip in `initSwipeNav`/`activePane`, index.html ~2275); `navigate(±1)` already branches for `artView`. Render `.md` + clean `.html` INLINE (no iframe) so they inherit the article swipe/N-P/mark-read flow; decide inline-vs-iframe BY CONTENT (full `<html>` doc → iframe; markdown/fragment → inline); `Inbox` newsletters keep the iframe. Also: delete advances to next item (not first article) + real delete via `DriveApp` write-back. Pairs with the rclone Wisdomware→Drive sync (`*.md`/`*.html` only). See CONTEXT "Wisdomware integration" + AUDIT 2026-06-29. |
